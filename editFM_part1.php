    ...
    if (Input::exists()) {
        if (Session::exists(Config::get("session/token_name")) && (Session::get(Config::get("session/token_name")) === Input::get('token'))) {
            if (Input::get('disabled')==='disabled') {
                    Redirect::to("rapport_HSE.php",array('visit=true&id'=>Input::get('id')));
            }
            $validate = new Validate();
            $check = array(
                'projet' => array("selected" => true),
                'operateur' => array("selected" => true),
                'chef_projet' => array("selected" => true),
                'logistique' => array("selected" => true),
            );
            ...
            if ($validation->passed()) {
                try {
                    $fields = array(
                        'projet_id' => Input::get('FM_id'),
                        'mission_file' => json_encode($_POST),
                        'author' => $user->data()->username,
                        'chef_projet' => Input::get('chef_projet'),
                        'logistique' => Input::get('logistique'),
                        'operateur' => Input::get('operateur'),
                        'date' => date('Y-m-d H:i:s'),
                        'projet' => Input::get('projet')
                    );
                    if (Input::get('total')) {
                        $fields['total_espece'] = Input::get('total');
                    }
                    if (Input::get("editable") === "true") {
                        $fm = new FM(Input::get('id'));
                        unset($fields['author'],$fields['date']);
                        $fm->update($fields, Input::get('id'));
                        if ($fm->is_visiting()) {
                            Redirect::to("rapport_HSE.php",array('visit=true&id'=>Input::get('id')));
                        }
                        Redirect::to("rapport_HSE.php",array('id'=>Input::get('id')));
                    } else {
                        $fm = new FM();
                        $fm->create($fields);
                        Redirect::to("rapport_HSE.php",array('projet_id'=>Input::get('FM_id')));
                    }
                } catch (Exception $e) {
                    die($e->getMessage());
                }
...
if (Input::exists('get')) {
    ...
    if (Input::get('visit')==='true'){
        ...
        if ($fm->is_finished()==='1'&&$user->data()->username===$fm->data()->author){
            Redirect::to("editFM_part2.php",array('id'=>Input::get('id')));
        }
    }
...
<form action="" method="post" role="form">
    <input type="hidden" value="<?php echo Input::get('FM_id'); ?>" id="FM_id" name="FM_id">
    <input type="hidden" value="<?php echo ($editable )?Input::get('numProjet'): getId(Input::get('projet')); ?>" id="numProjet"
           name="numProjet">
    <input type="hidden" id="add_lines" name="add_lines" value=0>
    <input type="hidden" id="editable" name="editable" value="<?php echo $editable ?"true":"false"; ?>">
    <input type="hidden" id="lines" name="lines" value="<?php echo Input::get('add_lines'); ?>">
    <fieldset <?php disable(Input::get('id')) ;?>>
    <p class="float-right" id="FM_infos" name="FM_infos"><?php echo Input::get('FM_id'); ?>
    </p>
    <br>
    <br>
    <span class="col-2 float-right">
        ...
        <select class="custom-select form-control" id="departement" name="departement">
            <option>...</option>
            <?php options('departement','departement');?>
        </select>
    </span>
    <br>
    <br>
    ...
    <div class="col-4">
            ...
            <select class="custom-select form-control" id="chef_projet" name="chef_projet">
                <option selected>...</option>
                <?php users('chef_projet'); ?>
            </select>
        </div>
        <div class="col-4">
            <label for="logistique">Responsable logistique</label>
            <select class="custom-select form-control" id="logistique" name="logistique">
                <option selected>...</option>
                <?php users('logistique'); ?>
            </select>
        </div>
    </div>
    <br>
                ...
                <td>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="dtlead"
                               name=<?php echo $name = "dtlead"; ?> value="<?php echo Input::get($name);
                                 $name = ""; ?>">
                        <label for="dt">DT</label>
                        <select class="custom-select form-control" id="dt" name="dt">
                            <option selected>...</option>
                            <?php users('dt'); ?>
                        </select>
                        <table class="table table-bordered" id="drivertable">
                            ...
                            <tr>
                                <td>
                                    <select class="custom-select form-control" id="driver1"
                                            name=<?php echo $name1 = "driver1"; ?>>
                                        <option selected>...</option>
                                        <?php users('driver1'); ?>
                                    </select>
                                </td>
                        ...
                        <button type="button" class="btn btn-secondary btn-lg btn-block " id="driveradd">add a
                            Driver
                        </button>
                    </div>
                </td>
                ...
                <td>
                    <div class="row">
                        <div class="form-group col-md-4 offset-md-2">
                            ...
                            <select class="custom-select form-control" id="nature_vehicule1" name="nature_vehicule1">
                                <option selected>...</option>
                        ...
                        <div class="form-group col-md-4 offset-md-2">
                            ...
                            <select class="custom-select form-control" id="nature_vehicule2" name="nature_vehicule2">
                                <option selected>...</option>
                                <?php options('nature_vehicule','nature_vehicule2');?>
                            </select>
                            <label for="type">Type</label>
    ...
    <div>
        <table class="table table-bordered">
            <thead class="thead-dark">
            <tr>
                <th scope="col" class="text-center">Frais Previsionnels</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                        ...
                        <tr>
                            <td>
                                <table class="table" id="carburant">
                                    ...
                                    echo " <option selected >...</option>";
                                    options('paiement_carburant',$newname);
                            ...
                            <td>
                                <table class="table" id="peage">
                                    ...
                                    echo " <option selected >...</option>";
                                    options('paiement_peage',$newname);
                            ...
                            <td>
                                <table class="table" id="repas">
                                    ...
                                    echo "name='", $newname, "' >";
                                    echo " <option selected >...</option>";
                                        options('paiement_repas',$newname);
                        ...
                        <tr>
                            <td>
                                <table class="table" id="hotel">
                                    ...
                                    echo "name='", $newname, "' >";
                                    echo " <option selected >...</option>";
                                        options('paiement_hotel',$newname);
                            ...
                            <td>
                                <table class="table" id="autres">
                                    ...
                                    echo "name='", $newname, "' >";
                                    echo " <option selected >...</option>";
                                    options('description_autres_frais',$newname);
                                    ...
                                    $newname = substr($name3, 0, strlen($name3) - 1) . $num;
                                        ...
                                        echo "name='", $newname, "' >";
                                        echo " <option selected >...</option>";
                                        options('paiement_autres_frais',$newname);
                        ...
                        <tr>
                            ...
                            <td>
                                <table class="table">
                                    ...
                                    <tr>
                                        <th scope="row">Montant Avancé</th>
                                        <td><input type="text"
                                                   name=<?php echo $name = "montant_avance"; ?> value="<?php echo Input::get($name);
                                                   $name = ""; ?>"
                                                   class="form-control" id="montant_avance"
                                            >
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row">Date</th>
                                        <td><p><?php if (isset($fm)) {echo escape($fm->data()->date) ;}?></p></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </td>
    ...
    <button type="submit" class="btn btn-primary  btn-lg btn-block" >Next
    </button>
</form>
    <script type="text/javascript" src="getweres1.js"></script>
<?php
