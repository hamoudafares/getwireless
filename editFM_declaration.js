...
function addLine(tableau){
    ...
    for (var cell of row) {
        ...
        if (newcell.tagName=="SPAN"){
           var  name = newcell.firstElementChild.name ;
            name = name.substring(0,name.length-1)+length;
            var incell1 = newcell;
            incell1.innerHTML="<input type=\"checkbox\" class=\"custom-control-input form-control\"\n" +
                "  id="+name+" name="+name+">                    " +
                " <label class=\"custom-control-label\" for="+ name+">Oui</label>";
            console.log(incell1);
        }else
        {
